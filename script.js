...
// const quickAddNoteBtn = document.getElementById('quick-add-note-btn'); // Removed
...
const updateNoteData = (noteIndex, newData) => {
    ...
    // NEW: Clean notebookId (ensure it's null or a number)
    const cleanNotebookId = (notebookId === null || notebookId === '' || notebookId === 'null' || isNaN(parseInt(notebookId))) ? null : parseInt(notebookId);
    ...
    // NEW: Check if notebookId changed
    if (note.notebookId !== cleanNotebookId) {
        note.notebookId = cleanNotebookId;
        changed = true;
    }

    if (changed) {
        note.lastModified = Date.now();
        saveNotes(); // Save all changes including notebookId
        return true;
    }
    return false;
};

// MODIFIED: Không tự động lưu notebookId khi autosave
const debouncedAutoSave = debounce((noteElement, noteIndex) => {
    ...
    // const editNotebookSelectCheck = noteElement.querySelector('select.edit-notebook-select'); // Không cần check select ở đây

    // Chỉ cần check các input cơ bản
    ...
    const newTagString = editTagsInputCheck.value;
    ...
    // const selectedNotebookId = editNotebookSelectCheck.value; // Không lấy notebookId ở đây

    const wasPreviouslyEmpty = !notes[noteIndex]?.title?.trim() && !notes[noteIndex]?.text?.trim();
    const isNowEmpty = !newTitle.trim() && !newText.trim();

    if (!wasPreviouslyEmpty && isNowEmpty) {
        // Nếu ghi chú không trống trước đó và giờ trống thì không auto-save
        // Người dùng phải nhấn Lưu để xác nhận xóa nội dung
        return;
    }

    // Chỉ cập nhật title, text, tags, color khi auto-save
    const saved = updateNoteData(noteIndex, {
        title: newTitle,
        text: newText,
        tags: newTags,
        color: newColor,
        notebookId: notes[noteIndex].notebookId // GIỮ NGUYÊN notebookId hiện tại khi auto-save
    });
...
// MODIFIED: Thêm logic tạo và chèn dropdown sổ tay
const handleNoteEdit = (noteElement, noteId, noteIndex) => {
    if (isViewingArchived || isViewingTrash) return; // Không cho sửa ở chế độ xem Lưu trữ/Thùng rác

    // Ngăn sửa nhiều ghi chú cùng lúc
    ...
    hideTagSuggestions(); // Ẩn gợi ý tag nếu có
    if (sortableInstance) sortableInstance.option('disabled', true); // Tắt kéo thả khi sửa
    showAddPanelBtn.classList.add('hidden'); // Ẩn nút FAB (+)

    const noteData = notes[noteIndex];
    if (!noteData) return; // Không tìm thấy dữ liệu ghi chú

    // Lưu lại HTML của các nút hành động gốc (trừ nút Lưu nếu có)
    ...
    if (actionsElementOriginal) {
        originalActionsHTML = Array.from(actionsElementOriginal.children)
            .filter(btn => !btn.classList.contains('save-edit-btn')) // Loại bỏ nút Lưu cũ nếu có
            .map(btn => btn.outerHTML)
            .join('');
    }

    // Tạo các input/textarea
    ...
    editInput.rows = 5; // Số dòng mặc định
    ...
    noteElement.dataset.selectedColor = noteData.color || ''; // Lưu màu hiện tại vào dataset
    NOTE_COLORS.forEach(color => {
        const swatchBtn = document.createElement('button');
        swatchBtn.type = 'button';
        swatchBtn.classList.add('color-swatch-btn');
        swatchBtn.dataset.colorValue = color.value || ''; // Lưu giá trị class màu
        ...
        if (color.value) {
            ...
            swatchBtn.innerHTML = '&#x2715;'; // Dấu X cho màu mặc định
        ...
        swatchBtn.addEventListener('click', () => {
            ...
            // Cập nhật trạng thái selected cho các nút màu
            colorSelectorContainer.querySelectorAll('.color-swatch-btn').forEach(btn => {
                const isSelected = btn === swatchBtn;
                btn.classList.toggle('selected', isSelected);
                btn.setAttribute('aria-checked', isSelected ? 'true' : 'false');
            });
            // Áp dụng màu ngay lập tức để xem trước (không lưu vào data chính)
            applyNoteColor(noteElement, { ...noteData, color: selectedValue });
            // Trigger auto-save (chỉ lưu các trường khác, màu sẽ được lưu khi nhấn Save chính)
            debouncedAutoSave(noteElement, noteIndex);
        });
        colorSelectorContainer.appendChild(swatchBtn);
    });

    // NEW: Tạo dropdown chọn sổ tay
    const notebookSelectContainer = document.createElement('div');
    notebookSelectContainer.classList.add('form-group', 'edit-notebook-group'); // Thêm class để style nếu cần

    const notebookLabel = document.createElement('label');
    notebookLabel.htmlFor = `edit-notebook-select-${noteId}`;
    notebookLabel.textContent = 'Sổ tay:';
    notebookLabel.classList.add('sr-only'); // Ẩn label khỏi màn hình nhưng vẫn tốt cho accessibility

    const notebookSelect = document.createElement('select');
    notebookSelect.id = `edit-notebook-select-${noteId}`;
    notebookSelect.classList.add('edit-notebook-select'); // Class để lấy giá trị khi lưu

    // Thêm tùy chọn mặc định (Không thuộc sổ tay nào)
    const defaultOption = document.createElement('option');
    defaultOption.value = ""; // Giá trị rỗng cho không thuộc sổ tay nào
    defaultOption.textContent = "-- Không thuộc sổ tay nào --";
    notebookSelect.appendChild(defaultOption);

    // Thêm các sổ tay vào dropdown
    notebooks.sort((a, b) => a.name.localeCompare(b.name)).forEach(notebook => {
        const option = document.createElement('option');
        option.value = notebook.id;
        option.textContent = escapeHTML(notebook.name);
        notebookSelect.appendChild(option);
    });

    // Chọn giá trị hiện tại của ghi chú
    notebookSelect.value = noteData.notebookId === null ? "" : noteData.notebookId;

    notebookSelectContainer.appendChild(notebookLabel);
    notebookSelectContainer.appendChild(notebookSelect);
    // END NEW
    ...
    // Xóa nội dung cũ và thêm các thành phần sửa đổi
    ...
    if (bookmarkIcon) {
        noteElement.appendChild(bookmarkIcon); // Thêm lại icon ghim
    ...
    noteElement.appendChild(notebookSelectContainer); // NEW: Thêm dropdown sổ tay

    // Tạo lại container cho các nút hành động và thêm nút Lưu
    const editActionsContainer = document.createElement('div');
    editActionsContainer.classList.add('note-actions');
    editActionsContainer.innerHTML = originalActionsHTML; // Thêm lại các nút gốc (Pin, Archive, Delete)
    editActionsContainer.appendChild(saveBtn); // Thêm nút Lưu
    noteElement.appendChild(editActionsContainer);

    // Gán sự kiện cho các input để auto-save (trừ dropdown sổ tay)
    const triggerAutoSave = () => debouncedAutoSave(noteElement, noteIndex);
    ...
    editTagsInput.addEventListener('blur', handleTagInputBlur, true);
    editTagsInput.addEventListener('keydown', handleTagInputKeydown);

    // Focus vào ô tiêu đề
    editTitleInput.focus();
    editTitleInput.setSelectionRange(editTitleInput.value.length, editTitleInput.value.length); // Di chuyển con trỏ đến cuối
};


// MODIFIED: Đọc giá trị từ dropdown sổ tay và cập nhật vào data
const handleNoteSaveEdit = (noteElement, noteId, noteIndex) => {
    ...
    // NEW: Lấy dropdown sổ tay
    const editNotebookSelect = noteElement.querySelector('select.edit-notebook-select');

    if (!editTitleInput || !editInput || !editTagsInput || !editNotebookSelect) { // NEW: Check select
        console.error("Lỗi lưu: Không tìm thấy các thành phần sửa ghi chú.");
        displayNotes(searchInput.value); // Render lại để tránh lỗi
        return;
    }
    ...
    const selectedColorValue = noteElement.dataset.selectedColor ?? notes[noteIndex]?.color;
    const newColor = selectedColorValue;
    // NEW: Lấy giá trị notebookId từ dropdown
    const newNotebookIdValue = editNotebookSelect.value;
    const newNotebookId = newNotebookIdValue === "" ? null : parseInt(newNotebookIdValue); // Chuyển thành null nếu rỗng, hoặc số

    // Kiểm tra nếu ghi chú gần như trống
    const wasInitiallyEmpty = !notes[noteIndex]?.title?.trim() && !notes[noteIndex]?.text?.trim();
    const isNowEmpty = !newTitle.trim() && !newText.trim();
    if (!wasInitiallyEmpty && isNowEmpty) {
        if (!confirm("Ghi chú gần như trống. Bạn vẫn muốn lưu?")) {
            return; // Hủy lưu nếu người dùng không đồng ý
        }
    }

    // Cập nhật dữ liệu, bao gồm cả notebookId mới
    updateNoteData(noteIndex, {
        title: newTitle,
        text: newText,
        tags: newTags,
        color: newColor,
        notebookId: newNotebookId // NEW: Truyền notebookId mới
    });

    // Lấy dữ liệu đã cập nhật để render lại
    const updatedNoteData = notes[noteIndex];

    // Render lại thẻ ghi chú với dữ liệu mới
    const bookmarkIcon = noteElement.querySelector('.pinned-bookmark-icon'); // Giữ lại icon ghim
    noteElement.innerHTML = ''; // Xóa nội dung sửa đổi
    if (bookmarkIcon) noteElement.appendChild(bookmarkIcon); // Thêm lại icon ghim

    applyNoteColor(noteElement, updatedNoteData);
    applyPinnedStatus(noteElement, updatedNoteData, isViewingArchived, isViewingTrash);

    const titleEl = createNoteTitleElement(updatedNoteData, searchInput.value);
    if(titleEl) noteElement.appendChild(titleEl);

    const contentEl = createNoteContentElement(updatedNoteData, searchInput.value, noteElement);
    ...
    const actionsEl = createNoteActionsElement(updatedNoteData); // Tạo lại các nút hành động
    if(actionsEl) noteElement.appendChild(actionsEl);

    delete noteElement.dataset.selectedColor; // Xóa màu tạm đã lưu trong dataset
    hideTagSuggestions(); // Ẩn gợi ý tag
    if (sortableInstance) sortableInstance.option('disabled', false); // Bật lại kéo thả
    if (addNotePanel.classList.contains('hidden')) showAddPanelBtn.classList.remove('hidden'); // Hiện lại nút FAB (+)

    // Hiệu ứng báo lưu thành công
    noteElement.classList.add('note-saved-flash');
    setTimeout(() => {
        noteElement?.classList.remove('note-saved-flash');
    }, 600);

    // NEW: Có thể cần render lại toàn bộ displayNotes nếu việc thay đổi notebookId
    // làm ghi chú biến mất khỏi view hiện tại.
    // Hoặc chỉ cần xóa element nếu nó không còn thuộc view này.
    const currentIdNum = (currentNotebookId !== 'all' && !isNaN(parseInt(currentNotebookId))) ? parseInt(currentNotebookId) : currentNotebookId;
    if (currentIdNum !== 'all' && updatedNoteData.notebookId !== currentIdNum) {
        // Nếu đang xem 1 sổ tay cụ thể và ghi chú vừa được chuyển đi
        noteElement.remove(); // Xóa khỏi DOM
        // Có thể thêm thông báo nhẹ "Ghi chú đã được chuyển sang sổ tay khác."
    } else if (currentIdNum === 'all' && updatedNoteData.notebookId !== null && notes.some(n => n.notebookId === updatedNoteData.notebookId)) {
         // Nếu đang xem "Tất cả" và ghi chú được chuyển vào 1 sổ tay,
         // không cần làm gì thêm vì nó vẫn hiển thị trong "Tất cả".
    } else {
        // Trường hợp khác (vẫn ở lại view hiện tại), không cần làm gì thêm sau khi render lại noteElement.
    }
...
function createNoteTitleElement(note, filter) { const title = note?.title?.trim(); if (!title && !note?.text?.trim()) return null; const titleElement = document.createElement('h3'); titleElement.classList.add('note-title'); let titleHTML = title ? escapeHTML(title) : '<i>Ghi chú không tiêu đề</i>'; const lowerCaseFilter = (filter || '').toLowerCase().trim(); const isTagSearch = lowerCaseFilter.startsWith('#'); if (title && !isTagSearch && lowerCaseFilter) { try { const highlightRegex = new RegExp(`(${escapeRegExp(lowerCaseFilter)})`, 'gi'); titleHTML = titleHTML.replace(highlightRegex, '<mark>$1</mark>'); } catch(e) { console.warn("Lỗi highlight tiêu đề:", e); } } titleElement.innerHTML = titleHTML; return titleElement; }
...
// =====================================================================
//  Other Panel/Import/Export Handlers
// =====================================================================
...
// Quick Add Panel Handlers (Removed)
...
const setupGlobalKeydownListeners = () => {
    document.addEventListener('keydown', (event) => {
        ...
        // const isQuickAddPanelOpen = quickAddPanel.classList.contains('visible'); // Removed Check
        const isSuggestionBoxOpen = !!document.getElementById(SUGGESTION_BOX_ID);
        const isEditingNote = activeElement?.closest('.note')?.querySelector('.edit-input, .edit-title-input, .edit-tags-input, .edit-notebook-select') === activeElement; // MODIFIED: Include select
        ...
        // const isEditingQuickAdd = quickAddPanel.contains(activeElement); // Removed Check
        ...
        if (event.key === 'Escape') {
             ...
             // else if (isQuickAddPanelOpen) hideQuickAddPanel(); // Removed Handle Escape for Quick Add
        ...
        const isAnyModalOpen = isNoteModalOpen || isTemplateModalOpen || isSettingsModalOpen || isNotebookModalOpen; // Removed: quick add check
        ...
        if (isTyping && !isEditingNote && !isEditingTemplate && !isEditingNotebook) return; // Removed: quick add check
        ...
        if (isCtrlOrCmd && event.key.toLowerCase() === 'n') {
            ...
            // else if (isEditingQuickAdd) { event.preventDefault(); saveQuickNoteBtn.click(); } // Removed: Save Quick Add
...
const setupEventListeners = () => {
    ...
    // setupQuickAddListeners(); // Removed
    setupSearchListener();
